...
class Parameter(Enumerable[None], IParameter[None]):
    ...
    def Format(self, key: str, values: str|None) -> str: return key
    
    def Join(self, builder: IQueryBuilder, column: str) -> str: return self.Format(column, builder.JoinParameters(self.AsIterable()))
...
class _ColumnParameterBase[TKey, TOperand: IOperandValue](IterableBase[TOperand], _IColumnParameterBase[TKey, TOperand]):
    ...
    @final
    def GetOperand(self) -> TOperand: return self.__operand
    
    @final
    def Join(self, builder: IQueryBuilder, column: str) -> str: return self.Format(column, builder.JoinOperands(self.AsIterable()))
    
    @final
    def Format(self, key: str, values: str|None) -> str: return f"{key} {self._GetInnerContainer().GetValue()} {values}"
...
class RoutineParameter[T](Enumerable[T], IParameter[T]):
    ...
    @final
    def Format(self, key: str, values: str|None) -> str: return f"{key}({StringifyIfNone(values)})"
    
    @final
    def TryGetEnumerator(self) -> IEnumerator[T]|None: return self.__args.TryGetEnumerator()
...
class TableValueArgument[T](TableArgument[T]):
    ...
    @final
    def Render(self, builder: IQueryBuilder) -> str: return builder.GetParameter(self.GetValue())
class TableColumnArgument(TableArgument[IColumn]):
    def __init__(self, column: IColumn) -> None: super().__init__(column)
    
    @final
    def Render(self, builder: IQueryBuilder) -> str: return self.GetValue().ToString(builder.FormatTableName)
...
class TableParameter[T](Enumerable[ITableArgument[T]], ITableParameter[T]):
    def __init__(self, alias: str|None, arguments: Iterable[ITableArgument[T]]|None) -> None:
        ...
        self.__arguments: IEnumerable[ITableArgument[T]]|None = TryAsEnumerable(arguments)
