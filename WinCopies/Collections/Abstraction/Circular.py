...
class CircularAbstract[TItem, TList](TupleBase[TItem], Sequence[TItem], ICircularTuple[TItem], GenericConstraint[TList, ICircularTuple[TItem]]):
    ...
    @final
    def GetStart(self) -> int: return self._GetInnerContainer().GetStart()
...
class CircularEquatableTuple[T: IEquatableValue](CircularBase[T, ICircularEquatableTuple[T]], EquatableTuple[T], ICircularEquatableTuple[T], IGenericConstraintImplementation[ICircularEquatableTuple[T]]):
...
class CircularHashableTuple[T: IHashableValue](CircularBase[T, ICircularHashableTuple[T]], HashableTuple[T], ICircularHashableTuple[T], IGenericConstraintImplementation[ICircularHashableTuple[T]]):
    def __init__(self, items: ICircularHashableTuple[T]) -> None: super().__init__(items)
    
    @final
    def SliceAt(self, key: slice) -> IHashableTuple[T]: return self._GetContainer().SliceAt(key)
...
class _ReversedArray[T](ReversedArrayBase[T, ICircularArray[T], IArray[T]], SequenceAbstract[T], ICircularArray[T], IGenericSpecializedConstraintImplementation[ITuple[T], ICircularArray[T]]):
    def __init__(self, items: ICircularArray[T], factory: IResumableEnumeratorMonitor[T]) -> None: super().__init__(items, factory)
    
    def GetMutability(self) -> Mutability: return Mutability.FixedSize
    
    def GetStart(self) -> int: return self._GetContainer().GetStart()
    
    @final
    def AsReversed(self) -> ICircularArray[T]: return self._GetContainer()
    
    @final
    def _SliceAt(self, key: slice) -> IArray[T]:
        return self._GetSpecializedContainer().SliceAt(key)
    @final
    def SliceAt(self, key: slice) -> IArray[T]: return self.ToSlicedAt(key)
@final
class _ArrayUpdater[T](ValueFunctionUpdater[ICircularArray[T]]):
    ...
    def _GetValue(self) -> ICircularArray[T]: return _ReversedArray[T](self.__array, self.__factory)
...
class _ReversedList[T](ReversedListAbstract[T, ICircularList[T], IList[T]], MutableSequenceAbstract[T], ICircularList[T], IGenericSpecializedConstraintImplementation[ITuple[T], ICircularList[T]]):
    def __init__(self, items: ICircularList[T], factory: IResumableEnumeratorMonitor[T]) -> None: super().__init__(items, factory)
    ...
    def SliceAt(self, key: slice) -> IList[T]: return self.ToSlicedAt(key)
...
class _ListUpdater[T](ValueFunctionUpdater[ICircularList[T]]):
    ...
    def _GetValue(self) -> ICircularList[T]: return _ReversedList[T](self.__array, self.__factory)
