...
class TupleBase[TIn, TOut, TSequence: IStringable](TupleCollectionAbstract[TIn, TOut, TSequence]):
    ...
    @final
    def __getitem__(self, index: SupportsIndex|slice) -> TOut|SequenceBase[TOut]:
...
class Tuple[TIn, TOut](TupleCollection[TOut], TupleBase[TIn, TOut, ITuple[TIn]], IGenericConstraintImplementation[ITuple[TIn]]):
    ...
    @final
    def _GetContainer(self) -> ITuple[TIn]:
        return self.__items
...
class Array[TIn, TOut](ArrayBase[TIn, TOut, IArray[TIn]], ArrayCollection[TOut], IGenericSpecializedConstraintImplementation[ITuple[TIn], IArray[TIn]]):
    ...
    @final
    def _GetContainer(self) -> IArray[TIn]:
        return self.__items
...
class List[TIn, TOut](ArrayAbstract[TIn, TOut, IList[TIn]], Collection[TOut], MutableSequence[TOut], IGenericSpecializedConstraintImplementation[ITuple[TIn], IList[TIn]]):
    ...
    @final
    def __getitem__(self, index: SupportsIndex|slice) -> TOut|MutableSequenceBase[TOut]:
        return self._Convert(self._GetInnerContainer().GetAt(int(index))) if isinstance(index, SupportsIndex) else self.SliceAt(index).AsMutableSequence()
...
@final
class _ValueEnumerable[TKey: IEquatableItem, TValueIn, TValueOut](CountableEnumerable[TValueOut]):
...
@final
class _Enumerator[TKey: IEquatableItem, TValueIn, TValueOut](Enumerator[IKeyValuePair[TKey, TValueIn], IKeyValuePair[TKey, TValueOut]]):
...
class Dictionary[TKey: IEquatableItem, TValueIn, TValueOut](Selector[TValueIn, TValueOut, IDictionary[TKey, TValueIn]], DictionaryBase[TKey, TValueOut]):
    @final
    class _Cookie[_TKey: IEquatableItem, T, U](_ICookie[T, U]):
        def __init__(self, dic: Dictionary[_TKey, T, U]) -> None:
            super().__init__()

            self.__dic: Dictionary[_TKey, T, U] = dic
        
        def Convert(self, item: T) -> U:
            return self.__dic._Convert(item)
