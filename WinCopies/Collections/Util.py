...
def __Between(x: SupportsRichComparison, value: SupportsRichComparison, y: SupportsRichComparison, bx: bool, by: bool) -> bool:
    from WinCopies.Comparison import Between

    global __betweenComparisonDelegate

    __betweenComparisonDelegate = Between

    return Between(x, value, y, bx, by)
def __Outside(x: SupportsRichComparison, value: SupportsRichComparison, y: SupportsRichComparison, bx: bool, by: bool) -> bool:
    from WinCopies.Comparison import Outside

    global __outsideComparisonDelegate
...
def TryMove[T](l: MutableSequence[T], x: int, y: int) -> bool|None:
    def validateIndex(index: int) -> bool: return ValidateIndex(index, length)

    length: int = len(l)

    if validateIndex(x) and validateIndex(y):
        if x == y: return False

        item: T = l.pop(x)

        if y == length: l.append(item)
...
def TryInsert[T](l: MutableSequence[T], index: int, item: T) -> bool|None:
    length: int = len(l)
    ...
    if _Outside(0, index, length): return None
...
def __Bisect[TIn, TOut: SupportsRichComparison](l: Sequence[TIn], item: TOut, selector: Callable[[Sequence[TIn], int], TOut], right: bool, check: bool, low: int, high: int) -> DualValueNullableBool[int]|None:
    def getIndex() -> DualValueNullableBool[int]:
        ...
        return CreateDualValueNullableBool(low, contains() if check else None)
...
def __GetAt[T](items: Sequence[T], index: int) -> T:
    return items[index]
def __GetSelector[TIn, TOut](selector: Converter[TIn, TOut]) -> Callable[[Sequence[TIn], int], TOut]:
    return lambda items, index: selector(items[index])

def Bisect[T: SupportsRichComparison](l: Sequence[T], item: T, right: bool = False, low: int = 0, high: int = -1) -> int:
    return __GetBisectionIndex(__Bisect(l, item, __GetAt, right, False, low, high), low)
