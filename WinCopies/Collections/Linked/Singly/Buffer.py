...
class AbstractBuffer[T](AbstractList[T]):
    ...
    @abstractmethod
    def _OnSetFirst(self, first: INodeCookie[T], last: INodeCookie[T]) -> None:
        ...

class ReadOnlyBuffer[T](ReadOnlyListBase[T, IBuffer[T]], IReadOnlyBuffer[T], IGenericConstraintImplementation[IBuffer[T]]):
    def __init__(self, items: IBuffer[T]) -> None: super().__init__(items)
    
    @final
    def Move(self) -> bool|None: return self._GetContainer().Move()
...
class BufferBase[T](AbstractBuffer[T], IBufferBase):
    ...
    def Move(self) -> bool|None:
        ...
        if result[1] is None: return False
...
class _ReadOnlyBufferedQueueUpdater[T](SelectionUpdater[IBufferedQueue[T], IReadOnlyBufferedQueue[T]]):
    def __init__(self, items: IBufferedQueue[T], updater: Method[IFunction[IReadOnlyBufferedQueue[T]]]) -> None: super().__init__(items, updater)
    
    def _AsContainer(self, container: IBufferedQueue[T]) -> IReadOnlyBufferedQueue[T]: return _ReadOnlyBufferedQueue[T](container)
...
class _ReadOnlyBufferedStackUpdater[T](SelectionUpdater[IBufferedStack[T], IReadOnlyBufferedStack[T]]):
    def __init__(self, items: IBufferedStack[T], updater: Method[IFunction[IReadOnlyBufferedStack[T]]]) -> None: super().__init__(items, updater)
    
    def _AsContainer(self, container: IBufferedStack[T]) -> IReadOnlyBufferedStack[T]: return _ReadOnlyBufferedStack[T](container)
...
class BufferedQueue[T](QueueBase[T], Buffer[T], AbstractBufferedQueue[T], IBufferedQueue[T]):
    def __init__(self, values: Iterable[T]|None = None) -> None:
        def update(func: IFunction[IReadOnlyBufferedQueue[T]]) -> None: self.__readOnly = func
    ...
    @final
    def _GetLastCookie(self) -> INodeCookie[T]|None:
        return self.__last
    @final
    def _SetLast(self, node: INodeCookie[T]) -> None:
        self.__last = node
    ...
    def AsReadOnly(self) -> IReadOnlyBufferedQueue[T]: return self.__readOnly.GetValue()
class BufferedStack[T](StackBase[T], Buffer[T], AbstractBufferedStack[T], IBufferedStack[T]):
    def __init__(self, values: Iterable[T]|None = None) -> None:
        def update(func: IFunction[IReadOnlyBufferedStack[T]]) -> None: self.__readOnly = func
    ...
    @final
    def AsReadOnly(self) -> IReadOnlyBufferedStack[T]: return self.__readOnly.GetValue()

class IBufferedQueueCookie[T](IInterface):
    def __init__(self) -> None: super().__init__()
    
    @abstractmethod
    def GetLast(self) -> INodeCookie[T]|None:
        ...
    @abstractmethod
    def SetLast(self, node: INodeCookie[T]) -> None:
        ...
...
class _BufferedList[T](Buffer[T], IBufferedList[T]):
    ...
    class _Cookie[_T](IBufferCookie[_T]):
        ...
        def GetFirst(self) -> INodeCookie[_T]|None: return self.__buffer._GetFirstNode()
        def SetFirst(self, node: INodeCookie[_T]) -> None: self.__buffer._SetFirstNode(node)
    ...
    class __Updater[_T](ValueFunctionUpdater[IBufferCookie[_T]]):
        ...
        def _GetValue(self) -> IBufferCookie[_T]: return _BufferedList._Cookie(self.__buffer)
...
class _CookieBufferedQueue[T](BufferedQueue[T], _BufferedList[T], IBufferedQueueList[T]):
    ...
    class _QueueCookie[_T](Abstract, IBufferedQueueCookie[_T]):
        ...
        def GetLast(self) -> INodeCookie[_T]|None: return self.__buffer._GetLastNode()
        def SetLast(self, node: INodeCookie[_T]) -> None: self.__buffer._SetLastNode(node)
    ...
    class __Updater[_T](ValueFunctionUpdater[IBufferedQueueCookie[_T]]):
        ...
        def _GetValue(self) -> IBufferedQueueCookie[_T]: return _CookieBufferedQueue._QueueCookie(self.__buffer)
    
    def __init__(self, values: Iterable[T]) -> None: super().__init__(values)
    ...
    @final
    def _GetLastNode(self) -> INodeCookie[T]|None:
        return self._GetLastCookie()
    @final
    def _SetLastNode(self, node: INodeCookie[T]) -> None:
        self._SetLast(node)
...
class ReadOnlyCountableBufferBase[TItem, TList](ReadOnlyListBase[TItem, TList], IReadOnlyCountableBuffer[TItem], GenericSpecializedConstraint[TList, ICountableList[TItem], ICountableBuffer[TItem]]):
    ...
    @final
    def GetCount(self) -> int: return self._GetSpecializedContainer().GetCount()
    
    @final
    def Move(self) -> bool|None: return self._GetSpecializedContainer().Move()
class ReadOnlyCountableBuffer[T](ReadOnlyCountableBufferBase[T, ICountableBuffer[T]], CountableCollection, IGenericConstraintImplementation[ICountableBuffer[T]]):
    def __init__(self, items: ICountableBuffer[T]) -> None: super().__init__(items)
    
    @final
    def _AsSpecialized(self, container: ICountableBuffer[T]) -> ICountableBuffer[T]: return container
class ReadOnlyCountableEnumerableBuffer[T](ReadOnlyCountableBufferBase[T, ICountableEnumerableBuffer[T]], CountableEnumerableCollectionBase[T], IReadOnlyCountableEnumerableBuffer[T], IGenericConstraintImplementation[ICountableEnumerableBuffer[T]]):
    def __init__(self, items: ICountableEnumerableBuffer[T]) -> None: super().__init__(items)
    
    @final
    def _AsSpecialized(self, container: ICountableEnumerableBuffer[T]) -> ICountableBuffer[T]: return container
...
class _CountableBufferBase[TItem, TList](CountableCollectionAbstract[TItem, TList], BufferBase[TItem], ICountableBuffer[TItem], GenericSpecializedConstraint[TList, IList[TItem], IBufferedList[TItem]]):
    def __init__(self, values: Iterable[TItem]) -> None: super().__init__(self._CreateBuffer(values))
...
class _CountableBufferedQueueBuffer[T](_CookieBufferedQueue[T], _IBufferedQueue[T]):
    def __init__(self, values: Iterable[T]) -> None:
        def update(func: IFunction[IBufferCookie[T]]) -> None: self.__cookie = func
        def updateQueueCookie(func: IFunction[IBufferedQueueCookie[T]]) -> None: self.__queueCookie = func
    ...
    @final
    def GetCookie(self) -> IBufferCookie[T]: return self.__cookie.GetValue()
    @final
    def GetQueueCookie(self) -> IBufferedQueueCookie[T]: return self.__queueCookie.GetValue()
@final
class _ReadOnlyCountableBufferedQueueUpdater[T](SelectionUpdater[ICountableBufferedQueue[T], IReadOnlyCountableBufferedQueue[T]]):
    def __init__(self, items: ICountableBufferedQueue[T], updater: Method[IFunction[IReadOnlyCountableBufferedQueue[T]]]) -> None: super().__init__(items, updater)
    
    def _AsContainer(self, container: ICountableBufferedQueue[T]) -> IReadOnlyCountableBufferedQueue[T]: return _ReadOnlyCountableBufferedQueue[T](container)
...
class _ReadOnlyCountableBufferedStackUpdater[T](SelectionUpdater[ICountableBufferedStack[T], IReadOnlyCountableBufferedStack[T]]):
    def __init__(self, items: ICountableBufferedStack[T], updater: Method[IFunction[IReadOnlyCountableBufferedStack[T]]]) -> None: super().__init__(items, updater)
    
    def _AsContainer(self, container: ICountableBufferedStack[T]) -> IReadOnlyCountableBufferedStack[T]: return _ReadOnlyCountableBufferedStack[T](container)
...
class CountableBufferedQueue[T](_CountableBufferBase[T, IBufferedQueueList[T]], AbstractBufferedQueue[T], ICountableBufferedQueue[T], IGenericSpecializedConstraintImplementation[IList[T], IBufferedQueueList[T]]):
    def __init__(self, values: Iterable[T]) -> None:
        def update(func: IFunction[IReadOnlyCountableBufferedQueue[T]]) -> None: self.__readOnly = func
    ...
    @final
    def AsReadOnly(self) -> IReadOnlyCountableBufferedQueue[T]: return self.__readOnly.GetValue()
class CountableBufferedStack[T](_CountableBufferBase[T, IBufferedList[T]], AbstractBufferedStack[T], ICountableBufferedStack[T], IGenericSpecializedConstraintImplementation[IList[T], IBufferedList[T]]):
    def __init__(self, values: Iterable[T]) -> None:
        def update(func: IFunction[IReadOnlyCountableBufferedStack[T]]) -> None: self.__readOnly = func
    ...
    @final
    def AsReadOnly(self) -> IReadOnlyCountableBufferedStack[T]: return self.__readOnly.GetValue()

@final
class _IterableEnumerableBufferUpdater[T](SelectionUpdater[IReadOnlyEnumerableBuffer[T], Iterable[T]]):
    def __init__(self, value: IReadOnlyEnumerableBuffer[T], updater: Method[IFunction[Iterable[T]]]) -> None: super().__init__(value, updater)
    
    def _AsContainer(self, container: IReadOnlyEnumerableBuffer[T]) -> Iterable[T]: return CreateEnumerable(container)
...
class ReadOnlyEnumerableBuffer[T](ReadOnlyListBase[T, IEnumerableBuffer[T]], IReadOnlyEnumerableBuffer[T], IGenericConstraintImplementation[IEnumerableBuffer[T]]):
    def __init__(self, items: IEnumerableBuffer[T]) -> None:
        def update(func: IFunction[Iterable[T]]) -> None: self.__iterable = func
...
class _ReadOnlyEnumerableBufferedQueue[T](ReadOnlyEnumerableBuffer[T], IReadOnlyEnumerableBufferedQueue[T]):
    def __init__(self, items: IEnumerableBufferedQueue[T]) -> None: super().__init__(items)
@final
class _ReadOnlyEnumerableBufferedQueueUpdater[T](SelectionUpdater[IEnumerableBufferedQueue[T], IReadOnlyEnumerableBufferedQueue[T]]):
    def __init__(self, items: IEnumerableBufferedQueue[T], updater: Method[IFunction[IReadOnlyEnumerableBufferedQueue[T]]]) -> None: super().__init__(items, updater)
    
    def _AsContainer(self, container: IEnumerableBufferedQueue[T]) -> IReadOnlyEnumerableBufferedQueue[T]: return _ReadOnlyEnumerableBufferedQueue[T](container)

@final
class _ReadOnlyEnumerableBufferedStack[T](ReadOnlyEnumerableBuffer[T], IReadOnlyEnumerableBufferedStack[T]):
    def __init__(self, items: IEnumerableBufferedStack[T]) -> None: super().__init__(items)
@final
class _ReadOnlyEnumerableBufferedStackUpdater[T](SelectionUpdater[IEnumerableBufferedStack[T], IReadOnlyEnumerableBufferedStack[T]]):
    def __init__(self, items: IEnumerableBufferedStack[T], updater: Method[IFunction[IReadOnlyEnumerableBufferedStack[T]]]) -> None: super().__init__(items, updater)
    
    def _AsContainer(self, container: IEnumerableBufferedStack[T]) -> IReadOnlyEnumerableBufferedStack[T]: return _ReadOnlyEnumerableBufferedStack[T](container)
...
class EnumerableBufferedQueue[T](EnumerableQueueBase[T], EnumerableBufferAbstract[T], AbstractBufferedQueue[T], IEnumerableBufferedQueue[T]):
    def __init__(self, values: Iterable[T]) -> None:
        def update(func: IFunction[IReadOnlyEnumerableBufferedQueue[T]]) -> None: self.__readOnly = func
    ...
    @final
    def AsReadOnly(self) -> IReadOnlyEnumerableBufferedQueue[T]: return self.__readOnly.GetValue()
class EnumerableBufferedStack[T](EnumerableStackBase[T], EnumerableBufferAbstract[T], AbstractBufferedStack[T], IEnumerableBufferedStack[T]):
    def __init__(self, values: Iterable[T]) -> None:
        def update(func: IFunction[IReadOnlyEnumerableBufferedStack[T]]) -> None: self.__readOnly = func
    ...
    @final
    def AsReadOnly(self) -> IReadOnlyEnumerableBufferedStack[T]: return self.__readOnly.GetValue()
...
class _ReadOnlyCountableEnumerableBufferedQueue[T](ReadOnlyCountableEnumerableBuffer[T], IReadOnlyCountableEnumerableBufferedQueue[T]):
    def __init__(self, l: ICountableEnumerableBufferedQueue[T]) -> None: super().__init__(l)
@final
class _ReadOnlyCountableEnumerableBufferedQueueUpdater[T](SelectionUpdater[ICountableEnumerableBufferedQueue[T], IReadOnlyCountableEnumerableBufferedQueue[T]]):
    def __init__(self, items: ICountableEnumerableBufferedQueue[T], updater: Method[IFunction[IReadOnlyCountableEnumerableBufferedQueue[T]]]) -> None: super().__init__(items, updater)
    
    def _AsContainer(self, container: ICountableEnumerableBufferedQueue[T]) -> IReadOnlyCountableEnumerableBufferedQueue[T]: return _ReadOnlyCountableEnumerableBufferedQueue[T](container)
...
class _ReadOnlyCountableEnumerableBufferedStack[T](ReadOnlyCountableEnumerableBuffer[T], IReadOnlyCountableEnumerableBufferedStack[T]):
    def __init__(self, l: ICountableEnumerableBufferedStack[T]) -> None: super().__init__(l)
@final
class _ReadOnlyCountableEnumerableBufferedStackUpdater[T](SelectionUpdater[ICountableEnumerableBufferedStack[T], IReadOnlyCountableEnumerableBufferedStack[T]]):
    def __init__(self, items: ICountableEnumerableBufferedStack[T], updater: Method[IFunction[IReadOnlyCountableEnumerableBufferedStack[T]]]) -> None: super().__init__(items, updater)
    
    def _AsContainer(self, container: ICountableEnumerableBufferedStack[T]) -> IReadOnlyCountableEnumerableBufferedStack[T]: return _ReadOnlyCountableEnumerableBufferedStack[T](container)
...
class CountableEnumerableBufferBase[T](CountableEnumerableBufferAbstract[T, IEnumerableBuffer[T]], IGenericSpecializedConstraintImplementation[IEnumerableList[T], IEnumerableBuffer[T]]):
    def __init__(self, l: IEnumerableBuffer[T]) -> None: super().__init__(l)
...
class CountableEnumerableBufferedQueueAbstract[T](CountableEnumerableBufferBase[T], IBufferedQueue[T]):
    def __init__(self, values: Iterable[T]) -> None: super().__init__(EnumerableBufferedQueue[T](values))
    
    @abstractmethod
    def AsReadOnly(self) -> IReadOnlyCountableEnumerableBufferedQueue[T]:
        ...
class CountableEnumerableBufferedStackAbstract[T](CountableEnumerableBufferBase[T], IBufferedStack[T]):
    def __init__(self, values: Iterable[T]) -> None: super().__init__(EnumerableBufferedStack[T](values))
...
class _CountableEnumerableBufferedQueue[T](CountableEnumerableBufferedQueueAbstract[T]):
    def __init__(self, items: CountableEnumerableBufferedQueue[T], values: Iterable[T]) -> None:
        def update(func: IFunction[IReadOnlyCountableEnumerableBufferedQueue[T]]) -> None: self.__readOnly = func
    ...
    @final
    def AsReadOnly(self) -> IReadOnlyCountableEnumerableBufferedQueue[T]: return self.__readOnly.GetValue()
class _CountableEnumerableBufferedStack[T](CountableEnumerableBufferedStackAbstract[T]):
    def __init__(self, items: CountableEnumerableBufferedStack[T], values: Iterable[T]) -> None:
        def update(func: IFunction[IReadOnlyCountableEnumerableBufferedStack[T]]) -> None: self.__readOnly = func
    ...
    @final
    def AsReadOnly(self) -> IReadOnlyCountableEnumerableBufferedStack[T]: return self.__readOnly.GetValue()
...
class CountableEnumerableBufferedQueue[T](CountableEnumerableBuffer[T, CountableEnumerableBufferedQueueAbstract[T]], ICountableEnumerableBufferedQueue[T], IGenericSpecializedConstraintImplementation[ICountableBuffer[T], CountableEnumerableBufferedQueueAbstract[T]]):
    def __init__(self, values: Iterable[T]) -> None: super().__init__(_CountableEnumerableBufferedQueue[T](self, values))
    
    def AsReadOnly(self) -> IReadOnlyCountableEnumerableBufferedQueue[T]: return self._GetContainer().AsReadOnly()
...
class CountableEnumerableBufferedStack[T](CountableEnumerableBuffer[T, CountableEnumerableBufferedStackAbstract[T]], ICountableEnumerableBufferedStack[T], IGenericSpecializedConstraintImplementation[ICountableBuffer[T], CountableEnumerableBufferedStackAbstract[T]]):
    def __init__(self, values: Iterable[T]) -> None: super().__init__(_CountableEnumerableBufferedStack[T](self, values))
    
    def AsReadOnly(self) -> IReadOnlyCountableEnumerableBufferedStack[T]: return self._GetContainer().AsReadOnly()
